...
public class Borrow extends connecttodb {
    ...
    public String readwl(String bid) throws SQLException, ClassNotFoundException {// make list of users on waitlist
        String wlinfo = "";
        String sql = "SELECT * FROM waitlists where id ='" + bid + "' ORDER BY created_at ASC";// check using the book
                                                                                               // selected's id
    ...
    public int countwl(String bid) throws SQLException, ClassNotFoundException {// make list of users on waitlist
        int result = 0;
        String sql = "SELECT COUNT(email) FROM waitlists where id ='" + bid + "' ORDER BY created_at ASC";// check using
                                                                                                          // the book
                                                                                                          // selected's
                                                                                                          // id
        PreparedStatement stmt = getConnect().prepareStatement(sql);
    ...
    public String wlTop(String bid) throws SQLException, ClassNotFoundException {// get first user on the waitlist
        String topinfo = "";
        String sql = "SELECT email FROM waitlists where id ='" + bid + "' ORDER BY created_at ASC LIMIT 1";// check
                                                                                                           // using the
                                                                                                           // book
                                                                                                           // selected's
                                                                                                           // id
    ...
    public int checkTime(String uemail, String bid) throws SQLException, ClassNotFoundException {
        ...
        OffsetDateTime atime = OffsetDateTime.parse("2012-01-01T12:30:30+01:00");// random date just to set up variable
                                                                                 // for rent time
        String sql = "SELECT created_at FROM waitlists where email ='" + uemail + "' and id ='" + bid + "'";// check
                                                                                                            // using the
                                                                                                            // book
        ...
        while (list.next()) { // get the created_at timestamp from the database to compare to the current time
            atime = list.getObject("created_at", OffsetDateTime.class);// get object of the timestamp when they rented
                                                                       // the book OR joined the wl
        }
        long result = ChronoUnit.DAYS.between(instant, atime.toInstant());// return number of days between the rent day
                                                                          // and now
        int returnDate = 21 - (int) result;// get 21 days minus the difference to find the date its due back.
        return returnDate;//
    }

    public void rbook(Boolean Availability, int wl, String uemail, String bid)
            throws SQLException, ClassNotFoundException {
        String sql = "INSERT INTO waitlists (email,id) VALUES ('" + uemail + "','" + bid + "')";// create waitlist using
                                                                                                // current user's id &
                                                                                                // the book selected
        PreparedStatement stmt = getConnect().prepareStatement(sql);
        stmt.executeUpdate();// UPDATE DATABASE after joining a waitlist
        String sql2 = "UPDATE books SET availability = ?, wl = ? WHERE id ='" + bid + "'";// need to update availability
                                                                                          // AND wl
    ...
    public void jWl(int wl, String uemail, String bid) throws SQLException, ClassNotFoundException {
        String sql = "INSERT INTO waitlists (email,id) VALUES ('" + uemail + "','" + bid + "')";// join/create waitlist
                                                                                                // using current user's
                                                                                                // id & the book
                                                                                                // selected
        PreparedStatement stmt = getConnect().prepareStatement(sql);
        stmt.executeUpdate();// UPDATE DATABASE after joining a waitlist
        String sql2 = "UPDATE books SET wl = ? WHERE id ='" + bid + "'";// only need to update wl since book is staying
                                                                        // unavailable
        PreparedStatement stmt2 = getConnect().prepareStatement(sql2);
    ...
    public void editwl(String uemail, String bid) throws SQLException, ClassNotFoundException {
        String sql = "DELETE FROM waitlists WHERE email = '" + uemail + "' and id = '" + bid + "'";// deletes current
                                                                                                   // user's id & the
                                                                                                   // book selected from
                                                                                                   // waitlist
        PreparedStatement stmt = getConnect().prepareStatement(sql);
